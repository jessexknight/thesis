...
and whether or not partnership durations are effectively capped at 1 year (\sref{foi.prior.dur}).
Thus, I considered 4 prior approaches, plus the new proposed approach from \sref{foi.prop}
...
\end{itemize}
Overall, \rd resulted in the most \emph{consistent} bias \vs \np across groups and over time.
\begin{figure}
  \subcaptionoverlap
  \foreach \var in {raw,rel}{
  \begin{subfigure}{\linewidth}
    \includegraphics[width=\linewidth]{foi.ep.incidence.\var.pdf}
    \caption{\raggedright}\label{fig:foi.ep.incidence.\var}
  \end{subfigure}}
  \caption{HIV incidence among selected risk groups,
    estimated under different prior force of infection approaches (colours)
    \vs the new proposed approach using equal model parameters}
  \label{fig:foi.ep.incidence}
  \floatfoot{\fffoi; \ffpopz; \ffribbon.}
\end{figure}
\par
...
As hypothesized above, for equal parameters, the 1-year approaches (\ry,~\py) featured
the greatest proportions of transmission via main/spousal partnerships, and the least via sex work.
By contrast, the full-duration approaches (\rd,~\pd) featured
the smallest proportions transmitted via main/spousal partnerships, and the most via sex work.
The distribution under the proposed approach (\np) was in between these two extremes.
Interestingly, there were minimal differences in Figure~\ref{fig:foi.wiw.part} between
incidence rate (\rd,~\ry) and incidence proportion (\pd,~\py) approaches.
\begin{figure}
...
\end{figure}
% Model recalibration under each prior approach followed the methods in \sref{model.cal}
% to yield 1000 unique parameter sets
%---------------------------------------------------------------------------------------------------
...
\begin{equation}\label{eq:tpaf}
  ...
  ,\qquad
  \Omega_{jk}(t) = \int_{t_0}^{t} \lambda_{j,M_k=0}(\tau)\,S_j(\tau)\,d\tau
  ,\qquad
  t = t_0 + \Delta_t
\end{equation}
...
Like classic PAFs, TPAFs can sum to more than 100\% \cite{Rowe2004,Mishra2021a}.
\par
...
\begin{figure}
  \includegraphics[width=\linewidth]{tpaf.foi.\var.pdf}
