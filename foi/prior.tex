...
% TODO: \includegraphics[scale=1]{diag.inst} - where?
...
reduces transmission \vs a model without partnerships by adjusting for PTC.
...
for a shorter (blue) \vs longer (green) partnership.
...
more and more acts occur after transmission.
...
\footnote{In fact, further variations on \eqrefs{eq:B.wph}{eq:B.bph} have also been used.}
  % \sref{app.foi.prior} gives some examples.}
\begin{itemize}
...
by the (weighted) AM-GM inequality (see \sref{app.foi.proof.xph}) \cite{Aldaz2009}.
...
we can show that $\lambda^\ir \ge \lambda^\ip$ (see \sref{app.foi.proof.xph}).
\par
...
due to \emph{future} PTC across multiple partnerships.
This reduction is then redundant because
PTC \emph{within} partnerships are already captured via
the per-partnership probability of transmission, while
PTC \emph{between} partnerships are already captured via
...
(see also \sref{app.model.math.qp}).
