...
\floatfoot{
  ...
  \tnt[2]{We defined a \emph{non-linear model} as one where
  ...
  \tnt[3]{We defined a \emph{compartmental model} as one where
  ...
  \tnt[4]{SSA was defined based on the countries in
    ...
    (see Table~\ref{tab:sr.search} for full country list).
    Studies were included if the model was parameterized/calibrated to reflect
