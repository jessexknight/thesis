...
prevalence (\subref{fig:fit.prevalence}) and incidence (\subref{fig:fit.incidence})
among selected risk groups.
Figures \ref{fig:fit.prevalence1v2}~and~\ref{fig:fit.incidence1v2} similarly illustrate
...
\begin{figure}
  \subcaptionoverlap
  \foreach \var in {prevalence,incidence}{
    \begin{subfigure}{\linewidth}
      \centering\includegraphics[scale=.5]{fit.\var.base.all.pdf}
      \caption{\raggedright}\label{fig:fit.\var}
    \end{subfigure}}
  \caption{Modelled HIV prevalence and incidence among selected risk groups
    and associated calibration targets}
  \label{fig:fit.hiv}
  \floatfoot{\fffit; \ffribbon; \fftargets.}
\end{figure}
\par
...
(Figure~\ref{fig:fit.incidence}), corresponding to
...
accumulation of ``partnership-level herd effects'' (see \sref{foi.exp.model}).
These trends can be compared with HIV prevalence from Eswatini antenatal care clinics
over the same period (Figure~\ref{fig:fit.prevalence.anc}), which suggest similar trends.%
...
\begin{figure}
  \subcaptionoverlap
  \foreach \var in {diagnosed,treated.c,treated.u,vls.c,vls.u}{
  \begin{subfigure}{\linewidth}
    \centering\includegraphics[scale=.5]{fit.\var.base.all.pdf}
    \caption{\raggedright}\label{fig:fit.\var}
  \end{subfigure}}
...
transmitted from (\subref{fig:wiw.base.from}) and
acquired among (\subref{fig:wiw.base.to}) modelled risk groups.
...
\begin{figure}
  ...
  \caption{Absolute numbers and proportions of modelled yearly HIV infections
    (\subref{fig:wiw.base.from}) transmitted from and (\subref{fig:wiw.base.to}) acquired among
