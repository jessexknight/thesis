...
To reflect this uncertainty in the model, a beta distribution was fitted
...
as defined in \sref{model.par.sw.part}, the client population size $P_{s_{2}i_{34}}$
...
The correction for partnership duration explored in \sref{TODO}
was not applied to new sex work partnerships. % TODO: double check
\par
...
Sampling $W'_{2+}$ from a beta distribution with 95\%~CI (10,~27)\%,
...
a beta distribution with 95\%~CI (10,~17)\%, yielding
...
individuals having a main/spousal partnership from a beta distribution with 95\%~CI (25,~50)\%,
...
see \sref{app.model.math.dur} for an example and further discussion.
...
originate from the medium activity group (beta prior).
...
was sampled from a beta distribution with 95\%~CI (25,~90)\%.
%===================================================================================================
...
\footnote{``Higher-risk'' populations in \cite{Owen2017} were defined as:
  \shortquote{STI clinic patients, FSW, their clients, and HIV-infected individuals}.}
...
from a beta prior distribution with 95\%~CI (0.6,~16.5)\%,
...
Similar to challenges in estimating sex work duration (\sref{app.model.math.dur}),
...
the proportion of enduring partnerships $\rho$ versus time $t$:
...
the total numbers of sex acts per modelled partnership (see \sref{foi.TODO});
...
However, the proposed approach to modelling partnerships introduced in \sref{foi.TODO}
...
% TODO: consider adding exit from "k=1" state, rate proportional to HIV-attrib-mort of opposite sex
...
%---------------------------------------------------------------------------------------------------
\subsubsection{Classic $\epsilon$ Mixing}
...
%---------------------------------------------------------------------------------------------------
\subsubsection{Log-Linear Mixing}
...
group sizes (\eg due to HIV mortality) or numbers of partnerships offered (\eg see \sref{foi.TODO}),
