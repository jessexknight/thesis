...
See \sref{model.par.fsex} for sex act frequency within each partnership type.
%---------------------------------------------------------------------------------------------------
...
See \sref{model.par.tm.condom} for the proportions of sex acts where condoms are used
...
into groups with 0-1 \vs 2+ partners per year, as described in \sref{model.par.wp}.
...
(\eg polygynous union, non-polygynous union, not in a union, see \sref{model.par.wp})
...
As illustrated in \sref{app.model.math.jsam}, this sampling strategy
...
7\% times the proportion sexually active (see \sref{model.par.wp}). % TODO: (~) see code
